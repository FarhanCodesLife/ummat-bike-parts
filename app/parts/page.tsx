...
function PartsContent() {
  ...
  const priceOptions: PriceOption[] = bikeType === "125" 
    ? [{ key: "a125", label: "125-A" }, { key: "b125", label: "125-B" }]
    : [{ key: "a70", label: "70-A" }, { key: "b70", label: "70-B" }];

  const addToCart = (part: Part, price: number, qualityLabel: string) => {
    let finalPrice = price;
    if (finalPrice === 0) {
      const customPrice = prompt(`Enter price for ${part.name}:`, "0");
      if (customPrice === null || customPrice === "" || isNaN(Number(customPrice))) return;
  ...
  const filteredParts = bikeParts.filter((p: Part) => 
  ...
  if (!isLoaded) return (
    <div className="h-screen flex items-center justify-center font-bold text-slate-400 uppercase tracking-widest bg-[#f8fafc]">
      Loading Inventory...
    </div>
  );
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8 bg-[#f8fafc] text-slate-900 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div>
          ...
          <p className="text-slate-500 text-lg font-bold uppercase tracking-[0.2em]">Honda {bikeType}cc Portal</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="hidden md:block bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  <th className="p-4 text-left font-bold text-slate-400 uppercase text-lg tracking-widest">Description</th>
                  {priceOptions.map(p => (
                    <th key={p.key} className="p-4 text-center font-bold text-slate-400 uppercase text-lg tracking-widest">{p.label}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {filteredParts.map((part: Part) => (
                  <tr key={part.id} className="hover:bg-blue-50/20 transition-colors">
                    <td className="p-4 font-black text-slate-700 uppercase text-lg">{part.name}</td>
                      ...
                      return (
                        <td key={opt.key} className="p-4 text-center">
                          <button
                            ...
                            className={`min-w-[110px] py-2 rounded-lg font-black text-lg transition-all border ${
                              isSelected ? "bg-blue-600 text-white border-blue-600 shadow-md" : "bg-slate-50 text-slate-500 border-slate-200 hover:bg-slate-200"
                            }`}
                          >
          ...
          <div className="md:hidden space-y-3">
            {filteredParts.map((part: Part) => (
              <div key={part.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm space-y-3">
                <p className="font-black text-slate-800 uppercase text-lg tracking-tight border-b border-slate-50 pb-2">{part.name}</p>
                  ...
                  {priceOptions.map(opt => {
                    ...
                    return (
                      <button
                        key={opt.key}
                        onClick={() => addToCart(part, part[opt.key], opt.label)}
                        className={`py-3 rounded-xl font-black text-[16px] flex flex-col items-center justify-center gap-1 border transition-all ${
                          isSelected ? "bg-blue-600 text-white border-blue-600 shadow-lg scale-95" : "bg-slate-50 text-slate-500 border-slate-100"
                        }`}
                      >
                        <span className="opacity-60 text-[16px] uppercase">{opt.label}</span>
        ...
        <div className="lg:col-span-4 lg:sticky lg:top-8 h-fit">
          <div className="bg-white p-6 rounded-3xl shadow-xl border-t-4 border-blue-600">
            <h2 className="font-black uppercase italic tracking-tighter text-lg mb-4 flex justify-between items-center">
              Summary <span className="text-blue-600 not-italic text-sm">[{cart.length}]</span>
            </h2>
                ...
                cart.map(item => (
                  <div key={`${item.id}-${item.quality}`} className="bg-slate-50 p-3 rounded-xl border border-slate-100 flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => updateQty(item.id, item.price, -1)} className="w-6 h-6 bg-white border border-slate-200 rounded text-slate-500 font-bold">-</button>
            ...
            <div className="pt-4 border-t-2 border-slate-100">
              <div className="flex justify-between items-center mb-6">
                <span className="font-black text-slate-400 uppercase text-[10px]">Total</span>
                <span className="font-black text-xl text-slate-900">Rs {total.toLocaleString()}</span>
              </div>
              <button 
                onClick={nextStep} 
                disabled={cart.length === 0}
                className={`w-full py-4 rounded-2xl font-black uppercase tracking-widest text-lg shadow-xl transition-all ${
...
// 2. DEFAULT EXPORT WITH SUSPENSE
export default function PartsPage() {
  return (
    <Suspense fallback={<div className="h-screen flex items-center justify-center font-bold text-slate-400 uppercase tracking-widest bg-[#f8fafc]">Loading...</div>}>
      <PartsContent />
    </Suspense>
  );
}